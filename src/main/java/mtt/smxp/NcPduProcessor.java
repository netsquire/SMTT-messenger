...
public class NcPduProcessor extends PDUProcessor{


	/**
	 * Class <code>NcPDUProcessor</code> gets the <code>Request</code>
	 ...
	 */	
		 ...
		 */
		private int intMessageId = 2000;

		/**
		 * System id of this simulator sent to the ESME in bind response.
		 */
		private static final String SYSTEM_ID = "Smsc Nc";
		 ...
		 * @param session the session this PDU processor works for
		 ...
		 */
		public NcPduProcessor(SMSCSession session, ShortMessageStore messageStore, Table users) {
		...
		/*
		public NcPduProcessor(PDUProcessorGroup processors,
				ShortMessageStore messageStore2,
				DeliveryInfoSender deliveryInfoSender2, Table users2) {
		...
		*/
		...
		@Override
		public void clientRequest(Request req) {
			System.out.println("Got Client Request." 
						+ "\n Command Status: " +  req.getCommandStatus()
						+ "\n Response Command Status: " + req.getResponse().getCommandStatus()
						+ "\n Debug String: " + req.debugString()
						);
		}
		*/
		...
		public void clientRequest(Request request) {
			debug.write("NcPDUProcessor.clientRequest() " + request.debugString());
			Response response;
			int commandStatus;
		...
		public void clientResponse(Response response) {
			debug.write("NcPDUProcessor.clientResponse() " + response.debugString());
		...
		public void serverRequest(Request request) throws IOException, PDUException {
			debug.write("NcrPDUProcessor.serverRequest() " + request.debugString());
		...
		public void serverResponse(Response response) throws IOException, PDUException {
			...
			debug.write("NcPDUProcessor.serverResponse() " + response.debugString());
